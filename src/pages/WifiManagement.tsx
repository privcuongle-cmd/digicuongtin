...
import { Search, Plus, Wifi, User, Phone, MapPin, Calendar, Trash2, Edit3, MoreHorizontal, X, Save, Clock, ChevronLeft, ChevronRight, MessageCircle, PhoneCall, Copy, MoreVertical, ArrowLeft } from 'lucide-react';
...
export const WifiManagement: React.FC = () => {
  ...
  const handleEdit = (record: WifiRecord, forceEdit = false) => {
      ...
      <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm sm:p-4 animate-in fade-in duration-200">
        <div className="bg-[#f8f9fa] sm:bg-white w-full h-full sm:h-auto sm:max-w-md sm:rounded-3xl shadow-2xl overflow-hidden flex flex-col animate-in slide-in-from-bottom-4 duration-300">
          ...
          <div className="px-4 py-3 flex items-center justify-between bg-white border-b border-slate-100">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-lg font-bold text-slate-800">
                {editingRecord ? (isEditing ? 'Sửa Wifi' : 'Chi tiết wifi') : 'Thêm Wifi mới'}
              </h2>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-3">
            ...
            <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100 space-y-3">
              <div className="flex items-center justify-between border-b border-slate-50 pb-2">
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="text-[12px] font-bold text-slate-700 uppercase tracking-tight">Thông tin Wifi</span>
                </div>
                {!isEditing && editingRecord && (
                  <button 
                    onClick={() => setIsEditing(true)}
                    className="text-[11px] font-bold text-blue-600 bg-blue-50 px-2 py-1 rounded-lg"
                  >
                    Sửa
                  </button>
                )}
                {isEditing && <ArrowLeft size={14} className="text-slate-300 rotate-180" />}
              </div>

              <div className="space-y-3">
                <div>
                  <label className="text-[10px] text-slate-400 font-bold uppercase block mb-1 tracking-wider">Tên mạng (SSID) (user/pass)</label>
                  <input 
                    type="text"
                    className={`w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-2.5 text-[14px] font-black text-slate-800 focus:border-blue-300 outline-none transition-all ${!isEditing ? 'cursor-default' : ''}`}
                    placeholder="SSID / Pass..."
                    value={wifiName}
                    onChange={(e) => isEditing && setWifiName(e.target.value)}
                    readOnly={!isEditing}
                  />
                </div>
              </div>
            ...
            {/* Address Section */}
            <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100">
              <div className="text-[10px] font-bold text-blue-600 uppercase tracking-wider mb-2">Địa chỉ</div>
              <textarea 
          ...
          <div className="p-4 bg-white border-t border-slate-100 flex gap-3">
            ...
            {(!editingRecord || isEditing) && (
              <button 
                onClick={handleSave}
                className="flex-[2] bg-blue-600 hover:bg-blue-700 text-white px-4 py-3.5 rounded-2xl font-bold shadow-lg shadow-blue-200 transition-all active:scale-95"
              >
                {editingRecord ? 'Lưu thay đổi' : 'Lưu Wifi'}
              </button>
            )}
          </div>
        </div>
    ...
    <div className="flex flex-col gap-3 md:gap-6 p-3 md:p-0">
      <div className="flex items-center justify-between gap-2 md:gap-4">
        <div className="min-w-0">
          <h1 className="text-xl md:text-2xl font-bold text-slate-800 tracking-tight leading-tight">Quản lý Wifi</h1>
        ...
        >
          ...
          <span className="hidden sm:inline">Thêm wifi mới</span>
      ...
      <div className="bg-white rounded-xl md:rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <div className="md:hidden bg-slate-50 p-3 space-y-2">
          {paginatedRecords.length > 0 ? paginatedRecords.map((record, idx) => (
            ...
            >
              <div 
                className={`p-3 flex justify-between items-center cursor-pointer active:bg-slate-50 transition-colors ${expandedId === record.id ? 'bg-blue-50/20' : ''}`}
                onClick={() => setExpandedId(expandedId === record.id ? null : record.id)}
              >
              ...
              {expandedId === record.id && (
                <div className="px-3 pb-3 pt-2 border-t border-slate-50 bg-slate-50/30 animate-in slide-in-from-top-2 duration-200">
                  <div className="flex items-center gap-2 bg-white p-2.5 rounded-xl border border-slate-100 shadow-sm w-full">
                    ...
                    <span className="text-[13px] font-black text-slate-700 break-all leading-tight">{record.wifiName}</span>
                  </div>
                  {record.note && (
