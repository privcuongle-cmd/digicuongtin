...
import { X, Search, Upload, Image as ImageIcon, Trash2, CheckCircle2, Loader2, Filter, ChevronLeft, ChevronRight, Camera } from 'lucide-react';
...
export const ImageLibraryModal: React.FC<ImageLibraryModalProps> = ({ isOpen, onClose, onSelect, defaultFilename, initialCategory }) => {
  ...
  const processAndUpload = async (file: File) => {
    ...
    try {
      ...
      reader.onload = async (event) => {
        ...
        // Use defaultFilename if provided, otherwise use original file name
        const filename = defaultFilename ? (defaultFilename.endsWith('.jpg') ? defaultFilename : `${defaultFilename}.jpg`) : file.name;
        ...
        const res = await uploadImage(base64, filename, category);
        if (res) {
          ...
          <div className="w-full lg:w-80 bg-slate-50/80 border-b lg:border-b-0 lg:border-r border-slate-100 p-6 flex flex-col gap-6 shrink-0 lg:overflow-y-auto">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div 
                ...
                className="w-full min-h-[160px] border-2 border-dashed border-slate-200 rounded-[2rem] flex flex-col items-center justify-center bg-white hover:border-blue-400 hover:bg-blue-50/50 transition-all cursor-pointer group relative overflow-hidden shadow-sm shrink-0"
              >
                <input 
                  type="file" 
                  ref={fileInputRef} 
                ...
                {isUploading ? (
                  ...
                  <>
                    <div className="w-12 h-12 rounded-2xl bg-blue-50 flex items-center justify-center text-blue-600 group-hover:scale-110 group-hover:bg-blue-600 group-hover:text-white transition-all duration-300 shadow-sm">
                      <Upload size={24} />
                    </div>
                    <div className="mt-3 text-center px-4">
                      <p className="text-sm font-black text-slate-700 tracking-tight">Thư viện ảnh</p>
                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Kéo thả hoặc click</p>
                    </div>
                  </>
                )}
              </div>
              ...
              <div 
                ...
                className="w-full h-[120px] bg-emerald-50 border-2 border-emerald-100 rounded-[2rem] flex flex-col items-center justify-center hover:bg-emerald-100 hover:border-emerald-300 transition-all cursor-pointer group shadow-sm shrink-0"
              >
                <input 
                  type="file" 
                  ref={cameraInputRef} 
                  ...
                  capture="environment"
                />
                <div className="w-12 h-12 rounded-2xl bg-white flex items-center justify-center text-emerald-600 group-hover:scale-110 transition-all duration-300 shadow-sm">
                  <Camera size={24} />
                </div>
                <div className="mt-3 text-center px-4">
                  <p className="text-sm font-black text-emerald-800 tracking-tight">Chụp ảnh mới</p>
                  <p className="text-[10px] text-emerald-400 font-bold uppercase tracking-widest mt-1">Sử dụng Camera</p>
                </div>
              </div>
            </div>
